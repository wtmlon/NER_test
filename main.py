...
def stanford_ner_v1(filename, verbose=True, absolute_path=None):
    out = 'out.txt'
    ...
    for res in results_str:
        if len(res.strip()) > 0:
            ...
            if len(entity_name) > 0 and len(entity_type) > 0 and entity_type == 'ORGANIZATION':
...
def find_org(args):
    ...
    entities = stanford_ner_v1(filename, verbose)
